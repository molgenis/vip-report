...
public class Base85 {
  ...
  /**
   * This is a base class for encoding data using the Base85 encoding scheme, in the same style as
   * Base64 encoder. Just override {@link #getEncodeMap()} to create a fully functional encoder.
  ...
  public abstract static class Encoder {
    ...
    /**
     * Encode string data into Base85 string. The input data will be converted to byte using UTF-8
     * encoding.
     *
    ...
    /**
     * Encode the data as one block in reverse output order. This is the strict algorithm specified
     * by RFC 1924 for IP address encoding, when the data is exactly 16 bytes (128 bits) long.
     * Because the whole input data is encoded as one big block, this is much less efficient than
    ...
    /**
     * Encode part of data as one block in reverse output order into output array. This is the
     * strict algorithm specified by RFC 1924 for IP address encoding, when the data part is exactly
    ...
    protected int _encodeDangling(
        final byte[] encodeMap,
        final byte[] out,
        final int wi,
  ...
  /**
   * This class encodes data in the Base85 encoding scheme using the character set described by IETF
   * RFC 1924, but in the efficient algorithm of Ascii85 and Z85. This scheme does not use quotes,
   * comma, or slash, and can usually be used in sql, json, csv etc. without escaping.
   *
   * <p>Encoder instances can be safely shared by multiple threads.
   *
   * @see <a href="https://tools.ietf.org/html/rfc1924">https://tools.ietf.org/html/rfc1924</a>
   */
  ...
  /**
   * This class encodes data in the Base85 encoding scheme Z85 as described by ZeroMQ. This scheme
   * does not use quotes or comma, and can usually be used in sql, json, csv etc. without escaping.
   *
   * <p>Encoder instances can be safely shared by multiple threads.
   *
   * @see <a href="https://rfc.zeromq.org/spec:32/Z85/">https://rfc.zeromq.org/spec:32/Z85/</a>
   */
   ...
   * Supports "z" and "y" compression, which can be disabled individually. Line break is not
   * supported.
   *
   * <p>Encoder instances can be safely shared by multiple threads.
   *
   * @see <a href="https://en.wikipedia.org/wiki/Ascii85">https://en.wikipedia.org/wiki/Ascii85</a>
   */
  ...
  /**
   * This is a skeleton class for decoding data in the Base85 encoding scheme. in the same style as
   * Base64 decoder. Just override {@link #getDecodeMap()} and {@link #getName()} to create a fully
   * functional decoder. Decoder instances can be safely shared by multiple threads.
   */
  public abstract static class Decoder {
    ...
    public int calcDecodedLength(final byte[] data, final int offset, final int length) {
      if (length % 5 == 1) {
        throw new IllegalArgumentException(
            length + " is not a valid Base85/" + getName() + " data length.");
      }
    ...
    /**
     * Decode the data as one block in reverse input order. This is the strict algorithm specified
     * by RFC 1924 for IP address decoding, when the data is exactly 16 bytes (128 bits) long.
     *
     * @param data Byte data to encode
     * @return Encoded data in ascii encoding
     * @see <a href="https://tools.ietf.org/html/rfc1924">https://tools.ietf.org/html/rfc1924</a>
    ...
    /**
     * Decode part of data as one block in reverse input order into output array. This is the strict
    ...
    protected int _decodeDangling(
        final byte[] decodeMap,
        final byte[] in,
        final int ri,
        final ByteBuffer buffer,
        int leftover) {
      ...
      long sum = decodeMap[in[ri]] * POWER_4 + decodeMap[in[ri + 1]] * POWER_3 + 85;
      if (leftover >= 3) {
  ...
  /**
   * This class decodes data in the Base85 encoding using the character set described by IETF RFC
   * 1924, in the efficient algorithm of Ascii85 and Z85. Malformed data may or may not throws
   * IllegalArgumentException on decode; call test(byte[]) to check data if necessary. Decoder
  ...
  /**
   * This class decodes data in the Base85 encoding scheme Z85 as described by ZeroMQ. Malformed
   * data may or may not throws IllegalArgumentException on decode; call test(byte[]) to check data
   * if necessary. Decoder instances can be safely shared by multiple threads.
   *
   * @see <a href="https://rfc.zeromq.org/spec:32/Z85/">https://tools.ietf.org/html/rfc1924</a>
   */
  ...
  /**
   * This class decodes Ascii85 encoded data (Adobe variant without &lt;~ and ~&gt;). 'y' and 'z'
   * are always processed. This keep the decoder simple. Malformed data may or may not throws
   * IllegalArgumentException on decode; call test(byte[]) to check data if necessary. Decoder
   * instances can be safely shared by multiple threads.
   *
   * @see <a href="https://en.wikipedia.org/wiki/Ascii85">https://en.wikipedia.org/wiki/Ascii85</a>
  ...
  public static Encoder getRfc1924Encoder() {
    ...
    return RFC1924ENCODER; // No worry if multiple encoder is created in multiple threads. Same for
    // all.
  }
