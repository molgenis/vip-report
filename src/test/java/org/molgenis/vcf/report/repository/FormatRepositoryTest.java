...
class FormatRepositoryTest {
  ...
  void testInsertFormatData() throws SQLException {
    ...
    List<String> formatColumns = Arrays.asList("GT", "VIPC_S");
    ...
    when(fieldMetadatas.getFormat()).thenReturn(Map.of("GT", fmGT, "VIPC_S", fmAttr));
    ...
    when(genotype1.getAnyAttribute("VIPC_S")).thenReturn(List.of("testValue", ".", "otherValue"));
    when(genotype2.hasAnyAttribute(anyString())).thenReturn(false);

    formatRepository.insertFormatData(
        conn, vc, formatColumns, 1, fieldMetadatas, samples, true, Map.of());
  ...
  @Test
  void testInsertFormatDataMissingTree() throws SQLException {
    Connection conn = mock(Connection.class);
    PreparedStatement ps = mock(PreparedStatement.class);
    when(conn.prepareStatement(
            "INSERT INTO \"format\" (\"_variantId\", \"_sampleIndex\", \"GT\", \"VIPC_S\") VALUES (?, ?, ?, ?)"))
    ...
    FieldMetadatas fieldMetadatas = mock(FieldMetadatas.class);
    FieldMetadata fmGT = mock(FieldMetadata.class);
    FieldMetadata vipcAttr = mock(FieldMetadata.class);
    when(vipcAttr.getType()).thenReturn(ValueType.CATEGORICAL);
    when(fieldMetadatas.getFormat()).thenReturn(Map.of("GT", fmGT, "VIPC_S", vipcAttr));
