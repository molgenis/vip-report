...
class FormatRepositoryTest {
  ...
  void testInsertFormatData() throws SQLException {
    ...
    when(conn.prepareStatement(
        "INSERT INTO format (variantId, sampleIndex, GT, DP) VALUES (?, ?, ?, ?)")).thenReturn(ps);
    when(conn.prepareStatement("INSERT INTO gtType (id, value) VALUES (?, ?)")).thenReturn(
        lookupPs);
    ...
    verify(conn).prepareStatement(
        "INSERT INTO format (variantId, sampleIndex, GT, DP) VALUES (?, ?, ?, ?)");
  }
}
