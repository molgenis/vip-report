...
class DatabaseSchemaManagerTest {
    ...
    private static final String NESTED_TABLE_SQL = "CREATE TABLE info (id INTEGER PRIMARY KEY AUTOINCREMENT,variant_id INTEGER REFERENCES vcf(id),AA TEXT,NS INTEGER,AF TEXT,H2 TEXT,DP INTEGER,DB TEXT);";
    private static final String EXPECTED_INFO_TABLE = "CREATE TABLE info (id INTEGER PRIMARY KEY AUTOINCREMENT,variant_id INTEGER REFERENCES vcf(id),AA TEXT,NS INTEGER,AF TEXT,H2 TEXT,DP INTEGER,DB TEXT);";
    private static final String EXPECTED_FORMAT_TABLE = "CREATE TABLE format (id INTEGER PRIMARY KEY AUTOINCREMENT,sample_index INTEGER REFERENCES sample(sample_index),variant_id INTEGER REFERENCES vcf(id),HQ TEXT,GQ INTEGER,DP INTEGER,GT TEXT,GT_type TEXT);";
}
