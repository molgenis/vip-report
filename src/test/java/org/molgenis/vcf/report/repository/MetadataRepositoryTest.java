...
class MetadataRepositoryTest {
  ...
  void testInsertMetadata() throws Exception {
    ...
    when(conn.prepareStatement("INSERT INTO numberType (id, value) VALUES (?, ?)")).thenReturn(
        preparedNumberType);
    when(conn.prepareStatement("INSERT INTO fieldType (id, value) VALUES (?, ?)")).thenReturn(
        preparedFieldTypes);
    when(conn.prepareStatement("INSERT INTO valueType (id, value) VALUES (?, ?)")).thenReturn(
        preparedValueTypes);
    ResultSet resultSet = mock(ResultSet.class);
    when(resultSet.next()).thenReturn(true);
    when(preparedMetadata.getGeneratedKeys()).thenReturn(resultSet);
    when(conn.prepareStatement(INSERT_METADATA_SQL, Statement.RETURN_GENERATED_KEYS)).thenReturn(
        preparedMetadata);
